...
import scrumMaster, { getEntities as getScrumMasters } from 'app/entities/scrum-master/scrum-master.reducer';
...
export const Product = () => {
  ...
  const getAllEntities = (scrumMasterId = null, developerId = null) => {
    dispatch(
      getEntities({
        page: paginationState.activePage - 1,
        size: paginationState.itemsPerPage,
        sort: scrumMasterId || developerId ? null : `${paginationState.sort},${paginationState.order}`,
        scrumMasterId,
        developerId,
      })
    );
            ...
            <ReactSearchBox
              data={
                filterBy.key === 'SCRUM_MASTER'
                  ? scrumMasterList.map(scrumMasterInstance => ({ key: scrumMasterInstance.id, value: scrumMasterInstance.name }))
                  : developerList.map(developer => ({ key: developer.id, value: developer.name }))
              }
              onSelect={({ item }) => {
                filterBy.key === 'SCRUM_MASTER' ? getAllEntities(item.key, null) : getAllEntities(null, item.key);
              }}
              onChange={() => {}}
